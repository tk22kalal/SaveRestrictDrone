...
async def login_cmd(event):
    ...
    async with Drone.conversation(event.chat_id, timeout=120) as conv:
        ...
        try:
            ...
            return await event.reply("Login timed out. Try again with /login.")
        ...
        if not phone.startswith("+"):
            return await conv.send_message(
                "Invalid phone number format. It must start with +.\nTry /login again."
            )
        ...
        try:
            ...
            return await conv.send_message(
                f"Too many requests. Try again after `{fw.value}` seconds."
            )
        except Exception as e:
            await acc.disconnect()
            return await conv.send_message(
        ...
        await conv.send_message(
            "OTP sent to your Telegram app / SMS.\n\n"
            "Send the OTP code now (e.g. `12345`).\n"
        ...
        tmp_session_file = f"login_tmp_{user_id}.session"
...
async def logout_cmd(event):
    ...
    session_file = f"login_tmp_{user_id}.session"
